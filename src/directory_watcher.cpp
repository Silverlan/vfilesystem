...
DirectoryWatcher::FileEvent::FileEvent(const std::string &fName)
	: fileName(fName),time(std::chrono::high_resolution_clock::now())
{}
...
DirectoryWatcherCallback::DirectoryWatcherCallback(const std::string &path,const std::function<void(const std::string&)> &onFileModified,WatchFlags flags)
	: DirectoryWatcher(path,flags),m_onFileModified(onFileModified)
{}

void DirectoryWatcherCallback::OnFileModified(const std::string &fName)
{
