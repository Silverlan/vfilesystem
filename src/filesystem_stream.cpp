...
fsys::StreamBuf::StreamBuf(std::size_t buff_sz,std::size_t put_back)
	: BaseStreamBuf(),
...
fsys::Stream::Stream(const char *filename,BaseStreamBuf *buf)
	: std::istream(buf)
{
...
fsys::Stream::Stream(const char *filename)
	: Stream(filename,new StreamBuf())
{}
fsys::Stream::~Stream()
{
	delete rdbuf();
