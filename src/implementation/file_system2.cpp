...
bool filemanager::clone_to_program_write_path(const std::string_view &path, bool overwriteIfExists)
{
...
bool filemanager::is_executable(const std::string_view &path)
{
#ifdef _WIN32
	return true;
#else
...
bool filemanager::make_executable(const std::string_view &path)
{
#ifdef _WIN32
	return true;
#else
...
void filemanager::find_files(const std::string_view &cfind, std::vector<std::string> *resfiles, std::vector<std::string> *resdirs, fsys::SearchFlags includeFlags, fsys::SearchFlags excludeFlags) { FileManager::FindFiles(cfind.data(), resfiles, resdirs, includeFlags, excludeFlags); }
void filemanager::find_system_files(const std::string_view &path, std::vector<std::string> *resfiles, std::vector<std::string> *resdirs, bool bKeepPath) { FileManager::FindSystemFiles(path.data(), resfiles, resdirs, bKeepPath); }
...
fsys::PackageManager *filemanager::get_package_manager(const std::string_view &name) { return FileManager::GetPackageManager(name.data()); }

bool filemanager::compare_path(const std::string_view &a, const std::string_view &b) { return FileManager::ComparePath(a.data(), b.data()); }
